...
type Logger interface {
	...
	SetErrorField(error string, object interface{})
...
func (l *logger) SetErrorField(error string, object interface{}) {
	l.SetField("error", error)
	if data, err := json.Marshal(object); err == nil {
		l.SetField("data", string(data[:]))
	}
}
...
func (l *logger) tagged() *logrus.Entry {
	if _, file, line, ok := runtime.Caller(2); ok {
		l.SetField("file", file+":"+strconv.Itoa(line))
	}
